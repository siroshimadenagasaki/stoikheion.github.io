...
function addSpaces(form, thingPressed) {
  ...
  if (form.yourSet.value.length != 0 && ((thingPressed >= 48 && thingPressed <= 57) || thingPressed == 69 || thingPressed == 84 || thingPressed == 101 || thingPressed == 116))
    form.yourSet.value += ' '
}
  ...
  for (
    ...
    if (field.charAt(i) != ' ') {
      switch (field.charAt(i)) {
        ...
        case '0':
          if (form.elements[0].checked == false) check(parseInt(field.charAt(i)))

          break

        case '1':
          if (form.elements[1].checked == false) check(parseInt(field.charAt(i)))

          break

        case '2':
          if (form.elements[2].checked == false) check(parseInt(field.charAt(i)))

          break

        case '3':
          if (form.elements[3].checked == false) check(parseInt(field.charAt(i)))

          break

        case '4':
          if (form.elements[4].checked == false) check(parseInt(field.charAt(i)))

          break

        case '5':
          if (form.elements[5].checked == false) check(parseInt(field.charAt(i)))

          break

        case '6':
          if (form.elements[6].checked == false) check(parseInt(field.charAt(i)))

          break

        case '7':
          if (form.elements[7].checked == false) check(parseInt(field.charAt(i)))

          break

        case '8':
          if (form.elements[8].checked == false) check(parseInt(field.charAt(i)))

          break

        case '9':
          if (form.elements[9].checked == false) check(parseInt(field.charAt(i)))
...
function allCheckboxestoText(form) {
  ...
  for (var i = 0; i < form.length; i++) {
    ...
    if (form.elements[i].type == 'checkbox' && form.elements[i].checked) {
      ...
      if (i <= 9) {
        if (form.yourSet.value.indexOf('' + i) == -1) form.yourSet.value += '' + i + ' '
      }

      if (i == 10) {
        if (form.yourSet.value.indexOf('t') == -1 && form.yourSet.value.indexOf('T') == -1) form.yourSet.value += 't '
      }

      if (i == 11) if (form.yourSet.value.indexOf('e') == -1 && form.yourSet.value.indexOf('E') == -1) form.yourSet.value += 'e '
    }
  }
}
...
function main(form, forteEntered) {
  ...
  if (forteEntered) {
    ...
    if (field.charAt(2) == 'Z') form.forte.value = form.forte.value.replace('Z', 'z')
  } else {
    var j = -1
  ...
  var transposed_set = transposeSet(normalForm, size, form.transpose.options[form.transpose.selectedIndex].value)

  form.transposeField.value = displayPcs(transposed_set, '[]', size)

  var inverted_set = invertSet(normalForm, size, form.inverse.options[form.inverse.selectedIndex].value)
  ...
  if (subSize >= size) {
    form.subResults.value = 'The subset cardinality can not be greater than or equal to the cardinality of your set'
  }

  if (superSize <= size) {
    form.superResults.value = 'The superset cardinality can not be less than or equal to the cardinality of your set'
  }
  ...
  if (subSize != 2 && subSize <= size) getSubsets(chords, primeForm, subSize, form)
...
function clearFields(form) {
  for (var i = 0; i < form.length; i++) {
    if (form.elements[i].type == 'text') if (form.elements[i] != form.yourSet) form.elements[i].value = ''
  }
}
...
function ZMate(primeForm, chord, size) {
  ...
  for (var i = 0; i < chord.length; i++) {
    ...
    if (areArraysEqual(candidateVector, iVector) && !areArraysEqual(primeForm, zMate_candidate)) return chord[i][0]
  }
...
function transposeSet(normalForm, size, amount) {
  ...
  for (i = 0; i < size; i++) new_set[i] = mod_12_add(parseInt(normalForm[i]), parseInt(amount))
...
function displayMatrixResults(normalForm, ySet, form, startingIndex) {
  ...
  for (var j = 0; j < ySet.length; j++) {
    for (var i = 0; i < normalForm.length; i++) {
      if (form.matrixOptions.value == 't' || form.matrixOptions.value == 't_invert') form.elements[k].value = mod_12_sub(normalForm[i], ySet[j])
...
function clearMatrix(form, length, startingIndex) {
  ...
  for (var i = startingIndex + length + 1; i <= startingIndex + length + 1 + length * 10; i += 10) {
    for (var j = i; form.elements[j].value != ''; j++) form.elements[j].value = ''
  }
...
function displaySubsetData(tally, form, chords) {
  ...
  for (var i = 1; i < tally.length; i++) {
    if (tally[i] != 0) form.subResults.value += chords[i - 1][0] + ': ' + tally[i] + '\n'
  }
}
...
function displaySupersetData(tally, form, chords) {
  ...
  for (var i = 1; i < tally.length; i++) {
    if (tally[i] != 0) form.superResults.value += chords[i - 1][0] + ': ' + tally[i] + '\n'
  }
}
  ...
  for (var i = 0; i < spaceremove.length; i++) {
    if (i == spaceremove.length - 1) {
      ...
      if (parseInt(spaceremove[i]) > parseInt(spaceremove[i + 1])) {
        interval[i] = 12 - parseInt(spaceremove[i]) + parseInt(spaceremove[i + 1])
      } else {
...
function updatecal(form) {
  ...
  var prime_value = form.prime.value.substring(form.prime.value.indexOf('[') + 1, form.prime.value.lastIndexOf(']'))
...
function keyToText(form, index) {
  if (document.forms[0].elements[index].checked == true) {
    ...
    if (index < 10) form.yourSet.value = form.yourSet.value.replace(index + ' ', '')
...
function check(index) {
  if (document.forms[0].elements[index].checked == true) document.forms[0].elements[index].checked = false
...
function selectCheckbox(value) {
  ...
  if (selectedValuesSet.has(value)) {
    ...
    document
      .querySelectorAll('.guitar-box-one-inner, .guitar-box-two-inner, .guitar-box-three-inner, .guitar-box-four-inner, .guitar-box-five-inner, .guitar-box-six-inner')
      .forEach((box) => {
...
function resetGuitarDisplay() {
  ...
  document.querySelectorAll('.selected-highlight').forEach((box) => box.classList.remove('selected-highlight'))
...
document.getElementById('downloadButtonbookabcd').addEventListener('click', showFields)
...
function hello1(number) {
  ...
  const checkboxId = `checkbox-${number === 't' ? '10' : number === 'e' ? '11' : number}`
